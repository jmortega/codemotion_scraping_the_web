...
#    br.set_debug_redirects(True)
    ...
    isLoginForm = lambda l: l.action == "ajax_login'" and l.method == "POST"

    try:
        if verbose: print 'selecting form...'
        #br.select_form(nr=0)
        #br.select_form(predicate=lambda f: f.attrs.get('action', None) == '/ajax_login')
        br.select_form(predicate=isLoginForm)
        for form in br.forms():
            print 'form'+str(form)
        #br.select_form(predicate=attrs(method='POST', action='/ajax_login'))
    except Exception,e:
        print e
